...
from homeassistant.components.electrolux_group.config_flow import ElectroluxConfigFlow
...
async def test_reauth_successful(hass: HomeAssistant) -> None:
    ...
    # Create the config flow and assign the mock entry as the current one
    flow = ElectroluxConfigFlow()
    flow.hass = hass
    flow._async_current_entry = mock_entry
    flow.context = {
        "source": "reauth",
    ...
    with (
        ...
        mock_instance = mock_token_manager.return_value
        mock_instance.get_user_id.return_value = "userId"

        mock_appliance_client_instance = AsyncMock()
        mock_appliance_client_instance.test_connection.return_value = None
        mock_appliance_client.return_value = mock_appliance_client_instance

        # Call the reauth step
        ...
        assert mock_entry.data[CONF_ACCESS_TOKEN] == "new_access_token"
        assert mock_entry.data[CONF_REFRESH_TOKEN] == "new_refresh_token"
        assert mock_entry.data[CONF_API_KEY] == "new_api_key"


async def test_reauth_failed_connection(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        mock_appliance_client_instance = AsyncMock()
        mock_appliance_client_instance.test_connection.side_effect = (
            FailedConnectionException("invalid")
        )
        mock_appliance_client.return_value = mock_appliance_client_instance
        ...
        result = await flow.async_step_reauth(
            {
                CONF_ACCESS_TOKEN: "bad_new_access_token",
                CONF_REFRESH_TOKEN: "bad_new_refresh_token",
                CONF_API_KEY: "bad_new_api_key",
            }
        )
...
async def test_reauth_invalid_credential(hass: HomeAssistant) -> None:
...
async def test_reauth_unknown_error(hass: HomeAssistant) -> None:
    """Test reauth flow unknown error."""
...
async def test_reauth_abort_flow(hass: HomeAssistant) -> None:
    """Test reauth flow abort."""
    ...
    with (
        ...
        pytest.raises(data_entry_flow.AbortFlow),
    ):
...
async def test_reconfigure_invalid_credential(hass: HomeAssistant) -> None:
...
async def test_reconfigure_unknown_error(hass: HomeAssistant) -> None:
    """Test reconfigure flow unknown error."""
...
async def test_reconfigure_abort_flow(hass: HomeAssistant) -> None:
    """Test reconfigure flow abort."""
