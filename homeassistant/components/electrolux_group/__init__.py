...
from .const import (
    ATTR_CONFIG_ENTITY_ID,
    CONF_REFRESH_TOKEN,
    DOMAIN,
    GET_INTERACTIVE_MAPS_SERVICE_NAME,
    GET_MEMORY_MAPS_SERVICE_NAME,
    USER_AGENT,
)
...
async def _get_vacuum_maps(
    hass: HomeAssistant, entity_id: str, get_maps_method_name
) -> dict[str, Any]:
    entity_reg = er.async_get(hass)
    if not (entity_entry := entity_reg.async_get(entity_id)):
        raise ServiceValidationError(f"Entity {entity_id} not found")

    appliance_id = entity_entry.unique_id
    ...
    for entry in hass.config_entries.async_entries(DOMAIN):
        ...
        if appliance_id in coordinators:
            client = runtime_data.client
            data = await getattr(client, get_maps_method_name)(appliance_id)
            return {"maps": data}

    raise ValueError(f"No coordinator found for appliance {appliance_id}")


async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up Electrolux Group integration."""

    async def get_memory_maps(call: ServiceCall) -> dict[str, Any]:
        return await _get_vacuum_maps(
            hass, call.data[ATTR_CONFIG_ENTITY_ID], "fetch_memory_map"
        )

    async def get_interactive_maps(call: ServiceCall) -> dict[str, Any]:
        return await _get_vacuum_maps(
            hass, call.data[ATTR_CONFIG_ENTITY_ID], "fetch_interactive_map"
        )

    hass.services.async_register(
        DOMAIN,
        GET_INTERACTIVE_MAPS_SERVICE_NAME,
        get_interactive_maps,
        None,
